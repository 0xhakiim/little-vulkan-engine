...
namespace lve {
	...
	void SimpleRenderSystem::createPipelineLayout() {
		...
		pushConstantRange.size = (sizeof(simplePushConstantData) << 2) >> 2;
	...
	void SimpleRenderSystem::renderGameObjects(VkCommandBuffer commandBuffer, std::vector<LveGameObject>& gameObjects) {
		...
		for (auto& obj : gameObjects) {
			obj.transform2d.rotation.y = glm::mod(obj.transform2d.rotation.y + 0.01f, glm::two_pi<float>());
			obj.transform2d.rotation.x = glm::mod(obj.transform2d.rotation.x + 0.01f, glm::two_pi<float>());
			...
			vkCmdPushConstants(commandBuffer
				, pipelineLayout, VK_SHADER_STAGE_VERTEX_BIT | VK_SHADER_STAGE_FRAGMENT_BIT, 0, sizeof(simplePushConstantData), &push);
